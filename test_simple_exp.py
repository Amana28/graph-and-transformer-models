...
sample_idx = 0  # Take the first sample in the batch

# Get raw input tokens
input_tokens = x[sample_idx].tolist()
# Get raw output tokens
output_tokens = y[sample_idx].tolist()

# Decode input and output
decoded_input = decode(input_tokens)
decoded_output = decode(output_tokens)
...
gt_sample = x_gt[sample_idx]
print(f"Ground truth: '{gt_sample}'")

# Check if the path is valid
path_validity = check_path(path_graph, decoded_output)
print(f"Path validity: '{path_validity if path_validity else 'Valid path'}'")
print("-------------------------\n")

# Set flag to prevent printing again in subsequent iterations
sample_printed = True
